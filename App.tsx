...
const App: React.FC = () => {
  ...
  if (isLoading) return html`
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 text-center">
      <${Lucide.Loader2} className="text-emerald-500 animate-spin mb-8" size=${60} />
  ...
  if (!user || !isAuthorized) {
    return html`<${LoginPage} onLogin=${handleFinalAuthorization} users=${users} onRegister=${() => {}} />`;
  }

  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: Lucide.LayoutDashboard },
    { id: 'property', label: 'Inventory', icon: Lucide.Home, hidden: user.role !== 'ADMIN' },
    { id: 'notices', label: 'Policies', icon: Lucide.ShieldCheck },
    { id: 'inbox', label: 'Security Mail', icon: Lucide.Mail },
    { id: 'profile', label: 'ID Settings', icon: Lucide.User },
    { id: 'admin', label: 'Terminal', icon: Lucide.Settings, hidden: user.role !== 'ADMIN' },
  ].filter(i => !i.hidden);

  const renderContent = () => {
    if (selectedFile) return html`<${AccountStatement} file=${selectedFile} onBack=${() => setSelectedFile(null)} />`;
    
    switch(currentPage) {
      case 'dashboard': return html`<${Dashboard} onSelectFile=${setSelectedFile} files=${user.role === 'ADMIN' ? allFiles : userFiles} userName=${user.name} />`;
      case 'property': return html`<${PropertyPortal} allFiles=${allFiles} setAllFiles=${setAllFiles} onPreviewStatement=${setSelectedFile} isLocalDataPinned=${isLocalDataPinned} />`;
      case 'notices': return html`<${PublicNotices} notices=${notices} />`;
      case 'inbox': return html`<${Inbox} messages=${messages} setMessages=${setMessages} currentUser=${user} onSendMessage=${(msg) => setMessages(prev => [...prev, msg])} users=${users} />`;
      case 'profile': return html`<${Profile} user=${user} onUpdate=${(u) => setUser(u)} />`;
      case 'admin': return html`<${AdminPortal} users=${users} setUsers=${setUsers} notices=${notices} setNotices=${setNotices} allFiles=${allFiles} setAllFiles=${setAllFiles} messages=${messages} onSendMessage=${(msg) => setMessages(prev => [...prev, msg])} onImportFullDatabase=${handleImportDatabase} onResetDatabase=${handleResetDatabase} isLocalDataPinned=${isLocalDataPinned} />`;
      default: return html`<${Dashboard} onSelectFile=${setSelectedFile} files=${userFiles} userName=${user.name} />`;
    }
  };
      ...
      <aside className=${`fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-slate-200 transition-transform duration-300 lg:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="h-full flex flex-col">
          ...
          <div className="p-6 border-t">
            <button onClick=${handleLogout} className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl text-sm font-black text-red-600 hover:bg-red-50 transition-colors"><${Lucide.LogOut} size=${20} /> Terminate</button>
          </div>
        </div>
      </aside>
      <main className=${`flex-1 flex flex-col min-w-0 lg:pl-72`}>
        <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b h-20 flex items-center px-4 lg:px-8 justify-between">
          <div className="flex items-center gap-4">
            <button onClick=${() => setIsSidebarOpen(true)} className="lg:hidden p-2.5 text-slate-900"><${Lucide.Menu} size=${24} /></button>
            ${isLocalDataPinned && html`<div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black uppercase border border-indigo-100">Local Cache Mode</div>`}
          </div>
