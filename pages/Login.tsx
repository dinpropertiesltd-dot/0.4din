  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'LOGIN') {
        ...
        const { data: existing, error: checkError } = await authProvider.checkCnicExists(regCnic);
        if (existing) {
          throw new Error(`This CNIC is already registered to ${existing.email}. Please login or use the correct email.`);
        }
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          required
                          placeholder="••••••••"
                          className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none text-sm"
                          value={regPassword}
